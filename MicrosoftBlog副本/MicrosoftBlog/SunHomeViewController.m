...
- (void)viewDidLoad
{
    ...
    _moreBtn.frame =CGRectMake(0, 0, 320, 40);
    _moreBtn.backgroundColor =[UIColor orangeColor];
    [_moreBtn setTitle:@"上拉刷新" forState:UIControlStateNormal];
    [_moreBtn addTarget:self action:@selector(upRefreshMoreData) forControlEvents:UIControlEventTouchUpInside];
...
- (void)request:(SinaWeiboRequest *)request didFinishLoadingWithResult:(id)result
{
    ...
    sqlite3 *sunDB = NULL;
    //创建fileNamePath
    NSString *filePath =[[NSBundle mainBundle]pathForResource:@"sun_Blog_client" ofType:@"sqlite"];
    //打开数据库
    int resultDB =sqlite3_open([filePath UTF8String], &sunDB);
    if (resultDB !=SQLITE_OK) {
        NSLog(@"dataBase open failed %@",filePath);
    }
    NSLog(@"%@",filePath);
   ...
   // NSLog(@"%@",result);
    ...
    if ([stringCompensionUrl isEqualToString:@"show.json"]) {
        ...
        //插入语句
        NSString *insertStr =@"insert into T_USER(id,user_id,screen_name) values (null,?,?)";
        sqlite3_stmt *stmt =NULL;
        sqlite3_prepare_v2(sunDB, [insertStr UTF8String], -1, &stmt, NULL);
        sqlite3_bind_int(stmt, 1, [[self.currentUserInfo objectForKey:@"id"]intValue]);
        sqlite3_bind_text(stmt, 2, [[self.currentUserInfo objectForKey:@"screen_name"]UTF8String], -1, NULL);
        resultDB = sqlite3_step(stmt);
        
    }else if([stringCompensionUrl isEqualToString:@"home_timeline.json"]){
        self.listStatuses =[result objectForKey:@"statuses"];
        NSLog(@"11111111111111111%@",self.listStatuses[0]);
        for (NSDictionary *status in self.listStatuses) {
            NSString *insertStatusStr =@"insert into T_STATUS(id,status_id,user_id) values (null,?,?)";
            sqlite3_stmt *stmt =NULL;
            sqlite3_prepare_v2(sunDB, [insertStatusStr UTF8String], -1, &stmt, NULL);
            sqlite3_bind_int(stmt, 1, [[status objectForKey:@"id"]intValue]);
            sqlite3_bind_text(stmt, 2, [[status objectForKey:@"mid"]UTF8String], -1, NULL);
          resultDB = sqlite3_step(stmt);
            
        }
        self.lastBlogID =[[self.listStatuses lastObject] objectForKey:@"uid"];
